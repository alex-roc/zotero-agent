...
right after the PyPI publish, in the same step that bumps `updates.json`.
    ...
    python scripts/gen_homebrew_formula.py --version 0.4.0 --sha256 <hex>
...
def sdist_url(version):
    """PyPI's stable per-project path, which 302s to the hashed one. Predictable
    from the version alone, so generating the formula needs no API call."""
    return ("https://files.pythonhosted.org/packages/source/z/zotero-agent/"
            "zotero_agent-%s.tar.gz" % version)
...
''' % {"desc": DESC, "homepage": HOMEPAGE, "url": sdist_url(version), "sha256": sha256,
...
def check():
    ...
    with open(OUT, encoding="utf-8") as fh:
        current = fh.read()
    declared = re.search(r'^  url ".*/zotero_agent-(.+?)\.tar\.gz"$', current, re.M)
    sha = re.search(r'^  sha256 "([0-9a-f]{64})"$', current, re.M)
    if not declared or not sha:
        problems.append("cannot read a version + sha256 out of the formula")
        return problems
    ...
    if current != render(version, sha.group(1)):
...
def main():
    ...
    ap.add_argument("--sha256", help="the published sdist's hash, when you have it without "
                                     "the file (e.g. from PyPI)")
    ...
    if not sha256:
        print("Need --sdist or --sha256: the hash comes from the published artifact, and "
              "sdists are not byte-identical across machines.", file=sys.stderr)
        return 1

    os.makedirs(os.path.dirname(OUT), exist_ok=True)
    with open(OUT, "w", encoding="utf-8") as fh:
        fh.write(render(version, sha256))
