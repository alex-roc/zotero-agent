...
def cmd_dedupe(args):
    ...
    code = (
        ...
        "  var used = {};\n"
        "  for (var a=0;a<singles.length;a++){ if(used[a]) continue; var grp=[groups[singles[a]][0]];\n"
        "    for (var b=a+1;b<singles.length;b++){ if(used[b]) continue; if(sim(singles[a],singles[b])>=threshold){ grp.push(groups[singles[b]][0]); used[b]=1; } }\n"
